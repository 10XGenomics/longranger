...
impl ThreadProxyIterator<Result<bam::record::Record, bam::ReadError>> {

    pub fn bam_file(f: String, buf: usize) -> ThreadProxyIterator<Result<bam::record::Record, bam::ReadError>> {
        let (tx, rx) = sync_channel::<Option<Result<bam::record::Record, bam::ReadError>>>(buf);
        let _ = thread::spawn(move || {
            let rdr = bam::Reader::from_path(f).ok().expect("erro opening BAM file");
