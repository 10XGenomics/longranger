...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    if bc_error_rate > 0.97:
        martian.exit("Extremely high rate of incorrect barcodes observed (%.2f %%). Check that input is 10x Chromium data, and that there are no missing cycles in the first 16bp of Read 1. Please note Long Ranger 2.0 and above do not support GemCode data." % (bc_error_rate * 100.0))
   ...
   # open sample_index file if there is one
    ...
    for read, barcode_read, sample_index_read in itertools.izip(read_iter, bc_iter, si_iter):
        ...
        if len(seq1) != len(qual1):
            martian.exit("Invalid FASTQ file: read and qual lengths don't match")
        ...
        if have_trim1 or have_trim2:
            tk_fasta.write_read_fastq(out_trim_fastq, name1, trim_seq1, trim_qual1)
        if paired:
            tk_fasta.write_read_fastq(out_read_fastq, name2, new_seq2, new_qual2)
            if have_trim1 or have_trim2:
                tk_fasta.write_read_fastq(out_trim_fastq, name2, trim_seq2, trim_qual2)
